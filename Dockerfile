...
RUN --mount=type=cache,target=${PIP_CACHE_DIR} \
  ...
  && uv pip install -r "./${PACKAGE_REQS}" --compile-bytecode --cache-dir "${UV_CACHE_DIR}" --python-preference=system --link-mode=copy
...
# COPY uv from Source
COPY --from=uv --chown=app:root --chmod=775 /uv /bin/uv
...
EXPOSE $PORT
CMD ["${VIRTUAL_ENV}/bin/python", "/app/launch.py"]
